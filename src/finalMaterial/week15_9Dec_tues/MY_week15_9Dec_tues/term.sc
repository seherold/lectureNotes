...
//Base case. multRec(x,0) fores to my base case (the if)
//and doesn't mke a recursive call. It terminates.

//Inductive step. We assume the inductive hypothesis -- that
//multRec(x,k) termiantes for some nonnegative integer k
//We must prove that multRec(x, k+1) terminates. We know
//k+1 is at least 1, so we would go into the else and make
//the resursive call multRec(x, k+1-1) which is multRec(x,k)
